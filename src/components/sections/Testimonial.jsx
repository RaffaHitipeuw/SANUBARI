...
export default function Testimonial() {
    ...
    return (
        <section className="relative items-center justify-center flex flex-col gap-12 bg-sariwhite py-28 overflow-hidden">
            ...
            <div className="relative flex items-center justify-center min-h-130 w-full px-4">
                <div className="relative w-180 min-h-130 flex items-center justify-center">
                    <button
                        onClick={prevCard}
                        className="navigation z-[999] absolute left-0 -translate-x-1/2 max-sm:translate-x-0 max-sm:bottom-0"
                        disabled={animating}
                    >
                        <ArrowLeft size={18} />
                    </button>
                    ...
                    <button
                        onClick={nextCard}
                        className="navigation z-[999] absolute right-0 translate-x-1/2 max-sm:translate-x-0 max-sm:bottom-0"
                        disabled={animating}
                    >
                        <ArrowRight size={18} />
                    </button>
                    {testimonials.map((item, index) => {
                        ...
                        return (
                            <div key={index} className={`testimonial absolute ${bgColor} ${zIndex} origin-center will-change-transform`} style={{transform, transitionProperty: "transform", transitionDuration: "900ms", transitionTimingFunction: "cubic-bezier(0.16, 1, 0.3, 1)",}}>
                                <div className="flex flex-col gap-2 w-1/2 max-sm:w-full z-2">
                                    <img src={item.image} alt={item.name} className="-mt-25 w-40 h-auto transition-all duration-700"/>
                                    <h1 className="text-sarired text-[40px] max-sm:text-3xl font-mr font-semibold transition-all duration-700">
                                        {item.name}
                                    </h1>
                                ...
                                <div className="flex flex-col justify-between w-1/2 max-sm:w-full relative overflow-clip rounded-r-[56px] transition-all duration-700 max-sm:gap-6 z-2">
                                    <p className="text-base/[145%] max-sm:order-2">
                                        {item.text}
                                    </p>
                                    <span className="testimonial-star max-sm:order-1">
                                ...
                                <div className="absolute top-0 left-0 w-full h-full overflow-hidden testimonial">
