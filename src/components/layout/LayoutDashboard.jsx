...
export default function LayoutDashboard() {
  ...
  return (
    <div className='flex gap-4 p-4 h-screen'>
      <header className={`${open ? 'w-[24vw]' : 'w-max'} flex flex-col gap-4 justify-between h-full`} onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)}>
        <nav className="overflow-clip border border-sariblack/14 h-full rounded-2xl flex flex-col items-stretch justify-between">
          <NavLink to={'/profile'} className='flex gap-4 items-center p-4'>
            <span className='block w-12 h-12 p-2 border border-sariblack/14 rounded-full bg-sariwhite'>
          ...
          <ul className='flex flex-col h-full gap-2 p-4'>
            {navItems.map((item, index) => (
              <li><NavLink className={'border border-transparent hover:border-sarired hover:bg-sarired/14 rounded-2xl hover:text-sarired transition duration-75 flex items-center px-4 h-14 gap-2 text-base'}><item.icon size={18} />{open && item.name}</NavLink></li>
            ))}
          </ul>
      ...
      <main className={`w-full h-full bg-sariblack`}>
        <Outlet/>
      </main>
    </div>
  );
